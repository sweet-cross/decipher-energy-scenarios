...
ZERO-Basisvariante (ZERO-Baseline Scenario)
Energy efficiency will be increased quickly and comprehensively, and the energy system 
will be highly electrified. Internal combustion engine vehicles will be replaced by 
electric vehicles, and fossil-fueled heating systems will be replaced by electric heat 
pumps and renewable energy-based heating networks. The use of biomass will increase 
significantly. Electricity-based energy sources (synthetic fuels and hydrogen) will 
also be used increasingly, but they won't play a more significant role until after 2045. 
Domestic electricity production from renewable energy sources will be expanded so 
quickly that a balanced annual balance will be achieved by 2050 (the "balanced annual 
balance 2050" scenario). This means that, over the course of a year, Switzerland can 
meet its electricity consumption with domestic electricity production. Residual 
greenhouse gas emissions in industry, waste management, and agriculture will 
be offset with CO2 capture and storage technologies and with negative emission 
technologies both domestically and abroad.

ZERO-Variante A (ZERO-Scenario A)
Same as the baseline scenario, but with even greater electrification of the energy system.

ZERO-Variante B (ZERO-Scenario B)
Same as the baseline scenario, but with less electrification of the energy system. 
Instead, biogas and synthetic gases, as well as hydrogen, will play a stronger role 
than in the baseline scenario.


ZERO-Variante C (ZERO-Scenario C)
Same as the baseline scenario, but with less electrification of the energy system. 
Instead, heating networks and liquid biogenic and synthetic fuels will play a stronger 
role than in the baseline scenario.

Scenario Variants:
   - KKW50: Extended nuclear operation to 2050
   - KKW60: Extended nuclear operation to 2060
...
"""
    ...
    async def process_query(
        ...
        comparison_data = await self._gather_comparison_data(
            query, scenarios_to_compare
        )

        # Perform scenario analysis
        analysis = await self._analyze_scenarios(
            query, scenarios_to_compare, comparison_data
        )
    ...
    def _identify_scenarios(self, query: str) -> List[str]:
        ...
        if (
            "zero" in query_lower
            or "net-zero" in query_lower
            or "net zero" in query_lower
        ):
            scenarios.append("ZERO-Basis")
        if (
            "wwb" in query_lower
            or "business as usual" in query_lower
    ...
    async def _gather_comparison_data(
        self, query: str, scenarios: List[str]
    ) -> Dict[str, Any]:
                ...
                for file_path, df in relevant_files.items():
                    if "scenario" in df.columns and "value" in df.columns:
                        ...
                        if not scenario_data.empty:
                            ...
                            comparison_data[variable][file_path] = (
                                scenario_data.to_dict("records")
                            )
    ...
    async def _analyze_scenarios(
        self, query: str, scenarios: List[str], comparison_data: Dict[str, Any]
    ) -> AgentResponse:
    ...
    def _calculate_scenario_confidence(
        self, scenarios: List[str], comparison_data: Dict[str, Any]
    ) -> float:
    ...
    def _generate_scenario_suggestions(
        self, query: str, scenarios: List[str]
    ) -> List[str]:
        ...
        # Time-based analysis
        suggestions.append(
            "Analyze the timeline and milestones for achieving scenario targets"
        )

        # Sectoral deep-dive
        if "transport" not in query.lower():
            suggestions.append(
                "Compare transport sector transformation across scenarios"
            )
        ...
        if len(scenarios) > 1:
            suggestions.append(
                "Assess risks and uncertainties for each scenario pathway"
            )
